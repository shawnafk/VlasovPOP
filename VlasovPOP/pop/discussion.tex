...
Note that $\mathrm{d}\mathcal{I} = \Omega \mathrm{d}\xi$, thus differential element $\mathrm{d}\psi = 1/\Omega \mathrm{d}\xi$, and we define
\begin{equation}
...
%The phase space area occupied by each adiabatic invariant is conserved.
%Once the phase space holes are initially formed, they propagate toward the equator with becoming deeper and deeper holes. The waterbag distribution is assumed to have a discontinuous jump at the separatrix from a constant trapped particle distribution to an equilibrium distribution. Therefore, the dynamics of waterbag is only determined from the separatrix and the jump of the distribution at the separatrix in terms of the local wave amplitude regardless of the history evolution of the waterbag, which is governed by the reduced Hamiltonian at the frame of the resonance.
